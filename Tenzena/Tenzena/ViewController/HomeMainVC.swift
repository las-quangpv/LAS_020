...
class HomeMainVC: BaseMainVC, IndicatorInfoProvider,UIScrollViewDelegate {
...
extension HomeMainVC:FSPagerViewDataSource,FSPagerViewDelegate {
    func pagerView(_ pagerView: FSPagerView, cellForItemAt index: Int) -> UICollectionViewCell {
        let cell = pagerView.dequeueReusableCell(withReuseIdentifier: "cell", at: index)
        let imageView = UIImageView(frame: CGRect(origin: CGPoint(x: 0, y: 0), size: pagerView.bounds.size))
        cell.addSubview(imageView)
        let item = listUpcomming[index]
        imageView.setImage(imageUrl: item.poster_path)
...
extension HomeMainVC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
...
extension HomeMainVC : GADNativeAdLoaderDelegate, GADAdLoaderDelegate {
